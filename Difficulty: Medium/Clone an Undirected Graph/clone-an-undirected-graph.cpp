...
class Solution {
  public:
    Node* dfs(Node* node, Node* cloneNode, map<Node*, Node*> &m) {
        for(auto it : node->neighbors) {
            if(m.find(it) == m.end()) {
                Node* adjCloneNode = new Node(it->val);
                m[it] = adjCloneNode;
                cloneNode->neighbors.push_back(adjCloneNode);
                dfs(it, adjCloneNode, m);
            }else{
                cloneNode->neighbors.push_back(m[it]);
            }
        }
        return cloneNode;
    }
