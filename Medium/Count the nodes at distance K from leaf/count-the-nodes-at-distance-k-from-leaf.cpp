...
{
    public:
    void inOrder(Node *root,int k,int level,vector<Node*> &vec,unordered_set<Node*> &s){
        if(root==NULL) return;
        if(root->left==NULL && root->right==NULL){
            if(k==0) s.insert(root);
            else if(level-k>=0) s.insert(vec[level-k]);
            return;
        }
        vec.push_back(root);
        inOrder(root->left,k,level+1,vec,s);
        inOrder(root->right,k,level+1,vec,s);
        vec.pop_back();
    }
